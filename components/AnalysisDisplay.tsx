...
export const AnalysisDisplay: React.FC<AnalysisDisplayProps> = ({ result }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      // Clonar el elemento para modificar estilos sin afectar la vista actual
      const clone = element.cloneNode(true) as HTMLElement;
      
      // Configurar estilos para permitir que el clon se expanda completamente
      // Esto es crucial para oraciones largas que requieren scroll horizontal
      clone.style.position = 'fixed';
      clone.style.top = '0';
      clone.style.left = '-10000px'; // Fuera de la pantalla
      clone.style.width = 'fit-content'; // Ajustar al contenido
      clone.style.height = 'auto';
      clone.style.maxWidth = 'none'; // Quitar restricciones de ancho máximo
      clone.style.margin = '0';
      clone.style.zIndex = '-1000';

      // Encontrar el contenedor con scroll y quitar el overflow
      ...
      // Añadir al DOM para que html2canvas pueda renderizarlo
      document.body.appendChild(clone);

      // Calcular dimensiones reales del contenido expandido
      // Añadimos un pequeño buffer para evitar cortes en los bordes
      ...
      // Lógica para ajustar la imagen a la página (Fit to Page)
      ...
      if (imgRatio > pageRatio) {
          // La imagen es más ancha en proporción que la página (ajustar al ancho)
          finalWidth = availableWidth;
          finalHeight = finalWidth / imgRatio;
      } else {
          // La imagen es más alta en proporción (ajustar al alto)
      ...
      pdf.save(`analisis_${cleanFileName}.pdf`);
    } catch (error) {
  ...
  return (
    <div className="mt-6 p-4 md:p-6 bg-slate-700/50 rounded-lg shadow-inner">
      ...
      {/* Contenedor de acciones (Botón descargar) */}
      ...
      {/* Área de captura: incluye todo el contenido relevante */}
      <div id="analysis-capture-target" className="p-4 bg-slate-800 rounded-md border border-slate-600">
        ...
        <div className="mb-4">
          ...
          <div className="overflow-x-auto pb-4 -mx-1 px-1">
              {/* Changed items-start to items-end to align Subject and Predicate at the bottom base */}
